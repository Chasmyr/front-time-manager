...
export default {
    ...
    methods: {
        test() {
            this.teams.map((team) => {
                if(this.teamsToDisplay.includes(team.id) && !this.teamsAlreadyDisplayed.includes(team.id)) {
                    let newKey = this.chartData[0].length
                    this.chartData[0][newKey] = 'Hours worked on average'
                    team.value.map((val, index) => {
                        this.chartData[index + 1][newKey] = val
                    })
                    this.teamsAlreadyDisplayed.push(team.id)
                }
            })
        }
    },
